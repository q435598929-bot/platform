...
public class SecurityConfig {
    @Bean
    SecurityFilterChain aiSecurity(HttpSecurity http, JwtAuthenticationConverter converter,
                                   BearerTokenResolver bearerTokenResolver) throws Exception {
        return http.cors(Customizer.withDefaults()).csrf(csrf -> csrf.disable())
                .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/swagger-ui/**", "/v3/api-docs/**", "/error").permitAll()
                        .requestMatchers(HttpMethod.POST, "/api/v1/chat/**", "/api/v1/conversations/**")
                            .hasAuthority("ai:execute")
                        .requestMatchers(HttpMethod.GET, "/api/v1/**").hasAuthority("ai:read")
                        .requestMatchers("/api/v1/**").hasAuthority("ai:write")
    ...
    @Bean
    NimbusJwtDecoder aiJwtDecoder(@Value("${platform.security.jwt-secret}") String encodedSecret,
    ...
    @Bean
    JwtAuthenticationConverter aiJwtAuthenticationConverter() {
